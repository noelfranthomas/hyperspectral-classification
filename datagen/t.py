...
tiff_files, class_defs = loadTiffFiles_datagen(train_subdirs,root_folder,1/4000,5)

# Generators
partition = math.floor(len(tiff_files) * train_rate)

training_generator = DataGenerator(tiff_files[partition:], class_defs[partition:], batch_size=batch_size)
validation_generator = DataGenerator(tiff_files[:partition], class_defs[:partition], batch_size=batch_size)
...
# Start training
model.fit(training_generator, validation_data=validation_generator,epochs=num_epochs,verbose=2)
