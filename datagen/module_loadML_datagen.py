...
class DataGenerator(tf.keras.utils.Sequence):
  def __init__(self, list_IDs, labels, img_mult_amt=1/4000, batch_size=10, dim=(1024,1024), n_channels=32, n_classes=2, shuffle=False):
      'Initialization'
      self.dim = dim
      self.batch_size = batch_size
      self.labels = labels
      self.list_IDs = list_IDs
      self.img_mult_amt = img_mult_amt
  ...
  def __data_generation(self, list_IDs_temp):
    ...
    for i, ID in enumerate(list_IDs_temp):
      ...
      for i in range(0,self.batch_size):
          print("Reading tiff file: "+str(self.list_IDs[i])+" with class label "+str(self.labels[i]))
          tiff_file = self.list_IDs[i]
          ...
          for j in range(32): # This will go through all 32 color channels
              ...
              img = np.array(imgTiff, dtype=np.float16)*self.img_mult_amt
              X[i,:,:,j] = img
              y[i] = self.labels[i]
