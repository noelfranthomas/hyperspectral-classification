...
def loadTiffFiles16(class_dir,root_folder, train_rate, np_precision, img_mult_amt=1, max_files=-1):
    ...
    X_train = np.empty((num_train, 1024, 1024, 32), dtype=np.float16)
    X_val = np.empty((num_val, 1024, 1024, 32), dtype=np.float16)
    Y_train = np.empty((num_train ), dtype=np.float16)
    Y_val = np.empty((num_val ), dtype=np.float16)
